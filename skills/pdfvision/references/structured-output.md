...
`text` is the pdfjs-derived text stream. Exact duplicate text draws with the same text, font, width/height, and text matrix are collapsed before joining so optional-content and overprint duplicates do not read as repeated words. Detected body-sized Japanese vertical columns are joined in source-stream order when that order matches the detected top-to-bottom column order; if a run disagrees, only that run falls back to the raw item join. On Japanese vertical body pages, adjacent half-size furigana/ruby columns are treated as annotations and excluded from `pages[].text` so the body flow stays readable. `--geometry` still exposes the raw glyph spans for inspection. This is a deliberate fidelity trade-off; pdfvision does not yet reattach ruby as inline `base《ruby》` annotations. `ocr.text` (when `--ocr` is on) is the OCR result alongside, **never overwriting `text`** — consumers diff or pick whichever signal looks better for the page.
