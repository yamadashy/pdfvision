...
- `raster_image_no_native_text` — a raster image dominates the page while native text is empty, so human-visible text inside the image will not appear in `pages[].text`; compare the render or OCR when exact text matters.
