...
- **Text/choice form field values are searched too**. Form-field matches come back with `source: 'formField'` and use the widget bbox, even when `--form-fields` was not requested for output. Internal values that are not visible text, such as unchecked checkbox `Off` or hidden/noView widget values, are not searched.
