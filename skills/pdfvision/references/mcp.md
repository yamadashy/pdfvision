...
- **Every successful result leads with an untrusted-data banner.** Error results do not, and can still quote the document — an unknown-attachment error lists the embedded filenames. MCP hosts have no equivalent of this skill, so the trust boundary travels with the payload.
