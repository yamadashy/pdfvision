...
|---|---|---|
| Reconstruct reading order, find headings, preserve table rows | `--layout` | Multi-column papers, including dense journal layouts with narrow repeated gutters or drop caps; Japanese vertical-writing slides/docs where CJK stacks surface as `writingMode: "vertical"`, plus display-spaced CJK title rows such as `科 学`; slides where the agent must process blocks in order; table-heavy financial/government PDFs where `layout.tables[]` row-major hints preserve numeric row/cell relationships better than raw blocks, including compact two-column year/value tables, dense recurring numeric gutters that would otherwise merge several values into one line, long financial row labels before recurring year columns, and leading header/sparse rows above wide recurring numeric tables with score/percentile cells |
...
| Find every occurrence of a string with bbox | `--search <query>` (repeatable; `--search-regex` / `--search-case-sensitive` modifiers) | The agent's "where does this term appear?" question. Returns `pages[N].matches[*]` with span/word/widget/annotation-level bbox so the bbox feeds straight into `--render-region` for a follow-up visual zoom — one-pipeline find-then-zoom, no second pass. Literal substring by default, case-insensitive, NFKC-aware (so `"fi"` matches the U+FB01 ligature), and CJK-aware enough that `科学` can match display-spaced `科 学`. Also searches visible text/choice form field values (match carries `source: 'formField'`), visible FreeText annotation contents (`source: 'annotation'`), and OCR text when `--ocr` is on (match carries `source: 'ocr'`, using OCR word boxes when present and supplementing from full `ocr.text` when word reconstruction misses one or more occurrences); duplicate OCR hits already covered by non-OCR matches are suppressed. |
...
- `dot_leader_noise` uses native text/layout signals and fires when many standalone dotted leader lines are extracted as separate text. Common cases: table-of-contents leaders and table leaders that visually connect labels to page numbers or values but can appear as noisy dot paragraphs in plain native text.
...
- `ocr_native_text_mismatch` appears when `--ocr` ran, OCR confidence is high, native extraction is otherwise `ok`, and the short OCR text strongly disagrees with native text. Common case: a custom font renders a word correctly while native text extracts as printable glyph substitutions; compare against `--render` before trusting exact text.
- `large_raster_low_text_overlap` can appear without `--image-boxes` on empty/sparse-text visual pages when pdfvision's internal image pass sees a large raster. With `--image-boxes` plus `--layout` or `--geometry`, it can also compare native text bboxes against raster regions and include `imageBoxIndex` for pinpoint follow-up. Treat it as "labels, chart text, map text, or screenshot text inside this image may need `--render` / `--render-region` / OCR."
- `annotation_text_missing_from_native` requires `--annotations` and appears when visible FreeText annotation contents are not represented in `pages[].text`. Treat native text as incomplete; read `pages[].annotations`, use `--search`, or render the annotation bbox when exact visible text matters.
