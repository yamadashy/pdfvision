...
| Open encrypted PDFs | `--password <value>` / `--password-stdin` | Password-protected PDFs when the user explicitly provides the document password. The password is only used for pdf.js decryption and is never emitted in output. Prefer `--password-stdin` when shell history or process argv exposure matters. Do not guess or store passwords. |
