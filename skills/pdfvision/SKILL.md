...
- `localized_glyph_noise` uses always-on text-quality signals and fires when several non-printable code points appear below the mixed-glyph ratio threshold, or when a CJK page contains isolated Latin-extended mojibake characters. Common cases: bullet symbols, dotted leaders, or custom icon fonts that render fine but extract as control characters or stray printable glyphs.
