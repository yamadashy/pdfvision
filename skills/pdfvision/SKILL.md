...
- `localized_glyph_noise` uses always-on text-quality signals and fires when multiple non-printable code points appear below the mixed-glyph ratio threshold, when native text contains Unicode replacement characters (`U+FFFD`), when a CJK page contains isolated Latin-extended mojibake characters, or when text is dominated by Latin-1 supplement printable mojibake. Common cases: formulas, comparison symbols, bullet symbols, dotted leaders, non-Latin custom fonts, or custom icon fonts that render fine but extract as control characters, replacement glyphs, or stray printable glyphs.
