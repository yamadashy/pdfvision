...
| Get crop-ready visual regions | `--visual-regions` | Figure/chart/diagram/table/form pages where an agent needs suggested `--render-region` bboxes, including nearby captions/form labels when found, instead of manually clustering raw image/vector/layout/form coordinates. Page-sized background boxes are suppressed when more specific foreground geometry exists, and dense vector grids can produce fallback regions for table-like structures. |
