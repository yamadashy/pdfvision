...
- Added `pdfvision docs` — the topic list and, with a topic name, its body — and cut `--help` from 20 KB to under 5 KB. The help was the first thing every agent read and the thing every error pointed at, so its size was a tax on first contact; the detail it carried now lives in thirteen topics that cost nothing until asked for. The topics are embedded at build time, so they describe the version actually installed rather than whatever the web has, and they need no network access. An unknown topic names the closest match and exits 1 instead of falling back to the list, which would read as "this topic is empty".
