...
| 7 | A real dynamic XFA (LiveCycle) form whose standard text layer is only the "Please wait…" viewer placeholder (e.g. `xfa_filled_imm1344e.pdf` from the pdf.js test corpus) | "What is this form for?" | The agent reports that the document is an XFA form whose real content pdfvision does not extract (guided by the `xfa_form` warning and/or `xfa: true`), and does not present the "Please wait… upgrade Adobe Reader" placeholder as the document's content. Naming the form from PDF metadata (e.g. the Title entry) is a bonus, not required. |
