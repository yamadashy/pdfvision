...
Every page reports `charCount`, `imageCount`, `vectorCount`, `textCoverage`, and `quality.nativeTextStatus`, so an agent can tell at a glance that "this slide is visual, not just text" or "this sparse OCR residue is not visible on the rendered page" — and decide to re-run with `--render` or `--ocr` instead of trusting an empty string or a lone page number.
