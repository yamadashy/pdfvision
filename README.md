...
| `search_pdf` | One row per distinct place a hit lands — page, origin, context, region, and a short `ref` — with occurrences that share a line or table row collapsed into a single `×N` row. Names the pages whose native text is unusable, so zero hits is never mistaken for absence. |
