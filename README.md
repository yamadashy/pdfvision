...
Each page can carry `pages[].warnings` — overlapping text, body running off the page, collisions with running headers/footers, glyph-garbage native text, localized glyph noise (including printable mojibake in CJK text), dense vector graphics such as form boxes or chart paths, numeric tables whose row/column relationships may be flattened, OCR/text layers over full-page scans, symbol-heavy OCR text layers, low-confidence OCR on scan-like pages, or large raster regions whose internal labels will not appear in native text — the "this looks off" cues a text-only extractor silently drops.
