...
每个成功输出的 TOON 在解码后都与 JSON 数据模型完全一致，未设置的 `undefined` 字段保持不存在。TOON 的字符串语法无法让未配对的 UTF-16 代理项无损通过 UTF-8 边界，因此 pdfvision 会拒绝该边界情况并提示使用 JSON，而不是静默替换字符。有效的代理项对和字面量 `\uD800` 文本不受影响。字段相同的对象数组可以使用只声明一次字段名的表格形式；形状统一的嵌套对象会折叠进表头（例如 `overview[]` 中的 `quality{nativeTextStatus}`）。

元素之间字段不同、含数组值字段，或嵌套对象形状不同的数组，仍使用列表形式。
