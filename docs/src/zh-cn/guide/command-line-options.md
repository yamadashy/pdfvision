...
选项描述如何读取 PDF。任何不读取 PDF 的操作都是子命令，只在第一个参数位置被识别，除 `--help` 外不接受任何属于自己的选项。`--help` 和 `--version` 是常见例外，在任何位置都有效。
...
如果文件恰好命名为 `mcp` 或 `clear-cache`，必须以 `./mcp` 或 `./clear-cache` 的形式传入。由于清理是破坏性操作，当工作目录中存在这样的文件时，`clear-cache` 会以退出码 `1` 拒绝执行，而不是猜测。
