...
20 页这道门槛决定的是返回 map 还是正文，而不是正文能不能放得下：不到 20 页的文档会被整篇读入，超出字符 budget 时同样会被截断。截断提示中被计为省略的是页面的*正文*——那些页面各自的 Overview 行仍然留在响应中，除非同一条提示里还写着 `Overview clipped after page N`——若没有任何一行完整地留下，则写作 `Overview clipped before any page row`。那是行本身也会一起丢失的唯一情形：区间宽到 Overview 表本身就用满了 budget 时，表格也会被截断，且一定落在行边界上。`after page N` 点名的是行仍然完整的最后一页，其后不再有逐页信息；`before any page row` 则表示一行都没有留下。
