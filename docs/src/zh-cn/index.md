...
hero:
  name: pdfvision
  text: 让悄无声息的 PDF 失败变得可以补救
  tagline: 空白扫描件、乱码字形、错乱的分栏，返回时都像是成功的结果。pdfvision 会提取文本、布局和页面图像，逐页标记问题，并指出下一步该怎么做。
  image:
    src: /logo.svg
    alt: pdfvision
  actions:
...
这个领域的多数工具瞄准的是转换：把 PDF 变成干净的 Markdown，然后期待结果足够忠实。pdfvision 瞄准的是诊断——它逐页报告提取结果是否可信，并在不可信的地方取回该处的视觉证据。
...
提取出问题时，页面自己会说出来。下面的例子里，文本层把一张图的标签解码成了乱码，而警告既解释了原因，也指出了应对办法：
