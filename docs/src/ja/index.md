...
hero:
  ...
  tagline: 気づけない PDF の失敗を、立て直せる失敗に。空のスキャン、化けた字形、崩れた段組みは、どれも成功したかのように返ってきます。pdfvision はテキスト、レイアウト、ページ画像を抽出し、問題をページ単位で警告し、次に取るべき手段まで示します。エージェントが気づかないまま誤答することをなくします。
  image:
    src: /logo.svg
    alt: pdfvision
  actions:
