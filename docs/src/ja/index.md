...
hero:
  ...
  tagline: PDF 抽出の気づけない失敗を、エージェントが立て直せるエラーに。抽出だけでは、空のスキャンも、化けた字形も、崩れた段組みも、成功として返ってきます——pdfvision はそれをページ単位で警告し、次の一手（ページのレンダリング、OCR、必要な領域だけのクロップ）まで示します。
  image:
    src: /logo.svg
    alt: pdfvision
  actions:
...
![pdfvision なしでは、エージェントは全ページを画像として運んでも表を読めないまま。pdfvision があれば search → render → verify で、根拠つきの回答にたどり着く。](/comparison.png)
