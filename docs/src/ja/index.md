...
この領域の多くのツールが目指しているのは変換です。PDF をきれいな Markdown に変え、その結果が忠実であることを願います。pdfvision が目指すのは診断です。抽出を信頼してよいかをページ単位で報告し、信頼できない場所ではその箇所に絞った視覚的な根拠を取りにいきます。
...
これは人間が PDF を読む流れに近いものです。ページをざっと見て、視覚的なページと抽出テキストが食い違う場所に気づき、答えを決めるグラフやフォーム欄を拡大する、という流れです。
...
抽出がうまくいかないときは、ページ自身がそう伝えます。次の例では図のラベルがテキスト層で文字化けしており、警告がその内容と対処法の両方を示しています。
