...
- `search_pdf(query: "…")` で語句を探す。同じ行や表の行に収まる出現は 1 行にまとめられ、`×N` で件数が示されます（見出しの件数は出現数のままです）。各行には `p47m1` のような短い `ref` が付くので、座標を書き写す代わりに `render_pdf(ref: "p47m1")` へそのまま渡してヒット箇所を目視できます。
