...
オプションは PDF の読み方を指定します。PDF を読まない操作はすべてサブコマンドで、先頭の引数としてのみ認識され、`--help` 以外の独自オプションは取りません。`--help` と `--version` は例外で、どこでも動作します。
...
`mcp` または `clear-cache` という名前のファイルを指す場合は `./mcp` または `./clear-cache` のように渡してください。削除は破壊的操作のため、作業ディレクトリにそのようなファイルがある場合、`clear-cache` は推測せずに終了コード `1` で拒否します。
