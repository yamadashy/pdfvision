...
サブコマンドは、オプション解析より前に、先頭の引数としてのみ認識されます。サブコマンドに未対応の引数を渡すと終了コード `1` です（`--help` と `--version` はそこでも有効です）。続いて CLI オプションの構文を解析します。不明なオプションや値が欠けたオプションは、`--help` があっても終了コード `1` です。構文解析に成功した後は、`--version`、`--help`、`--clear-cache` の順に処理します。これらはそこで終了し、入力確認と抽出オプションの意味検証を行いません。それ以外では、`--remote` の前後の空白を除去します。位置引数が複数あれば、入力元の有無を確認する前に終了コード `1` で終了します。位置引数が 1 個以下の場合は、空でない位置引数または空白でない `--remote` URL があるかを確認します。どちらもなければ、抽出、キャッシュ設定、抽出オプションの意味検証より前に完全な usage を stderr へ出し、終了コード `2` で終了します。利用可能な入力がある場合の引数検証エラーと、キャッシュ削除の失敗は終了コード `1` です。
...
| `--render-scale <n>` | `--render`, `--render-visual-regions`, `--ocr` のラスタライズ倍率を指定します。既定は `2`、範囲は `(0, 4]` です。3つのフラグのいずれも指定されていない場合はエラーになります。 |
...
| `--clear-cache` | `clear-cache` サブコマンドの非推奨エイリアスです。引き続きキャッシュを削除して警告を表示します。v1.0 で削除されます。 |
| `-v, --version` | pdfvision のバージョンを表示します。 |
| `-h, --help` | CLI ヘルプを表示します。 |

## サブコマンド

オプションは PDF の読み方を指定します。PDF を読まない操作はすべてサブコマンドで、先頭の引数としてのみ認識され、独自オプションは取りません。`--help` と `--version` は例外で、サブコマンドの後ろも含めどこでも動作します。

| サブコマンド | 用途 |
| --- | --- |
| `clear-cache` | pdfvision の所有権マーカーを検証した後にだけ、設定されたキャッシュルートを削除して終了します。危険な広域ルート、マーカーのないカスタムルート、その他の検証できないルートは拒否します。 |
| `mcp` | pdfvision を Model Context Protocol で stdio 上に提供します。[MCP サーバー](./mcp-server.md) を参照してください。 |

`mcp` または `clear-cache` という名前のファイルを指す場合は `./mcp` または `./clear-cache` のように渡してください。削除は破壊的操作のため、作業ディレクトリにその名前のエントリ（リンク先が存在しない symlink を含む）がある場合、`clear-cache` は推測せずに終了コード `1` で拒否します。
