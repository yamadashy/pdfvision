...
基本方針は **うまく読めなかったときは、そう伝え、次に何をすべきかも伝える** ことです。警告は症状と対処法をセットで示すため、フォントマップやコンテンツストリームの知識がなくても、エージェントは不完全な抽出に対処できます。pdfvision がネイティブテキストを OCR やレンダリング結果で勝手に置き換えることはありません。観測した事実を報告し、判断はエージェントに委ねます。
