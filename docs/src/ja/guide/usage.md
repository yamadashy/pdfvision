...
`--remote` は、ユーザーが別途許可した接続先にだけ使ってください。レスポンスは検証しますが、接続先そのものは検証せず、プライベートアドレスやリダイレクト先も遮断しません。信頼できない URL は直接渡さず、解決された各 IP とリダイレクト先を許可リストで検証して接続先を固定できる fetcher で取得し、ローカルファイルとして pdfvision に渡してください。または、pdfvision の取得処理をネットワーク制御の内側に隔離してください。詳しくは[セキュリティとプライバシー](./security-and-privacy.md)を参照してください。
