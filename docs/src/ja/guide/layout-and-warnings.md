...
`--geometry` は保持された位置付き pdf.js テキスト項目ごとに 1 つの span を `pages[].spans` に出します。項目は 1 文字、単語、より長い文字列の場合があり、隣接項目は別々のままです。bbox はグリフ輪郭ではなく、項目全体を囲む丸め済みの軸平行矩形です。各 span には、見出し判定などに使える概算フォントサイズ `fontSize` も含まれます。
