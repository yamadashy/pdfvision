...
每個成功輸出的 TOON 在解碼後都與 JSON 資料模型完全一致，未設定的 `undefined` 欄位保持不存在。TOON 的字串語法無法讓未配對的 UTF-16 代理項無損通過 UTF-8 邊界，因此 pdfvision 會拒絕此邊界情況並提示使用 JSON，而不會默默替換字元。有效的代理項配對和字面值 `\uD800` 文字不受影響。欄位相同的物件陣列可以使用只宣告一次欄位名稱的表格形式；形狀一致的巢狀物件會折疊進表頭（例如 `overview[]` 中的 `quality{nativeTextStatus}`）。

項目之間欄位不同、含陣列值欄位，或巢狀物件形狀不同的陣列，仍使用列表形式。
