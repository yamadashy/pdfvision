...
- `search_pdf(query: "…")` 定位一個詞。來源相同且裁切區域相同的多次出現（通常是同一列或同一表格列內的重複）會合併成一列，並用 `×N` 標出次數（標題處的計數仍然是出現次數）。每列都帶一個像 `p47m1` 這樣的短 `ref`——把它原樣傳給 `render_pdf(ref: "p47m1")`，就能就地查看匹配內容，不必抄寫座標。一個 source 的 ref 集合，來自它最近一次 `search_pdf` 或最近一次整頁 `render_pdf` 登記的內容；這兩者中任意一個都會整體取代掉先前的集合，而區域 render——包括所有帶 `ref` 的呼叫——不會新登記任何內容，原有集合保持不變，因此一次搜尋命中的結果可以逐一依序渲染。`ref` 不能與 `pages` 或 `region` 同時使用：ref 本身已經同時指定了頁面與區域，這類呼叫會被直接拒絕，而不是悄悄按 ref 所在的頁面來處理。
