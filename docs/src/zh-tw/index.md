...
hero:
  ...
  tagline: 把 PDF 擷取中悄無聲息的失敗，變成代理能夠補救的錯誤。只靠擷取本身，空白掃描件、亂碼字形、錯亂的分欄都會作為成功回傳——pdfvision 會逐頁標記它們，並指出下一步該怎麼做：渲染頁面、執行 OCR，或只裁切關鍵區域。
  image:
    src: /logo.svg
    alt: pdfvision
  actions:
...
![沒有 pdfvision，代理把每一頁都當圖片搬運，表格仍然讀不了；有 pdfvision，search → render → verify，給出帶證據的回答。](/comparison.png)
