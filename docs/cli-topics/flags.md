...
- A second budget bounds the whole regex request at ~12s, so a pattern that would otherwise cost pages × 1s on a long document stops instead of running for minutes. Matches found before the deadline are kept; the remaining pages are left unsearched and a single warning names how many pages were searched out of how many and the page range to resume from. That result is not cached. Treat its `0 matches` as "not searched", never as absence.
