...
The compact model also includes optional `pageDiagnostics: [{ page, quality, warnings? }]`. An entry is retained for every selected page with at least one warning, `quality.nativeTextStatus !== 'ok'`, or a defined non-`ok` `quality.visualStatus` — even when that page has no hit and even when `totalMatches` is zero. `quality` is the raw `PageQuality`, and `warnings` contains complete `PageWarning` objects, including optional `blockIndex`, `otherBlockIndex`, and `imageBoxIndex`; those indices refer to arrays available in the full report, which the compact projection still omits. A non-`ok` native status describes native text only: it does not imply that OCR, form fields, links, or annotations produced no hit. Healthy searches keep the previous shape because `pageDiagnostics` is omitted when no page qualifies.

When an XFA warning indicates that the searched page text may be a viewer placeholder rather than the document content, compact output also includes `unreadableSource: { pages, notes }`. Its pages preserve the document-wide scope of XFA warnings that are attached only to the first selected page, and its notes distinguish a confirmed placeholder from an unconfirmed one. In particular, rendering a confirmed XFA placeholder shows the placeholder too; open the file in Adobe Acrobat/Reader instead. JSON and decoded TOON use those fields directly. XML emits `<pageDiagnostics><page nativeTextStatus="..." visualStatus="..."><warnings>...</warnings></page></pageDiagnostics>` and `<unreadableSource>` page/note lists; Markdown shows `Diagnostics` and `Unreadable source` sections before any zero-match return. Inspect these fields before treating either a hit or a miss as visible evidence.
...
- **Native text is searched at reconstructed visual-line level**. A query can cross pdf.js span / font-run boundaries on the same line (e.g. `"Hello World"` split into `Hello` + `World`) and returns a union `bbox` plus per-span `boxes[]`. Narrow horizontal column gutters are treated as line breaks, so matches do not stitch the end of one horizontal column to the start of another.
- **Detected CJK vertical body text is searched in reading order**: top-to-bottom within a column, then right-to-left across columns. A query can cross those detected body columns. `pages[].text` joins the same columns only when the source stream already follows that order.
- **Unambiguously associated ruby appears inline as `base《ruby》`**. Native search considers both ruby-inclusive and ruby-stripped forms, so a base-word query can match through an annotation. Detected ruby that lacks an unambiguous base association remains excluded from native search lines, `pages[].text`, and layout.
- **General phrase stitching across horizontal prose lines is not supported** because the resulting region is usually too broad for visual zoom. Narrow exceptions join adjacent hyphenated terms (`exam-` + `specific` matches `exam-specific`) and short, geometrically aligned stacked labels (`Kansas` above `City` matches `Kansas City`). The hyphen is retained: `examspecific` is not a native-search variant. The full `SearchMatch` output retains per-contributor geometry in `boxes[]`.
- **A native hit's `context` quotes the reconstructed layout line when it covers the hit and contains the matched string**. This matters most for right-to-left scripts, where reconstruction restores the inter-word spaces pdf.js emits as separate items and un-mirrors bracket glyphs; the matched search line does neither. When no reconstructed layout line meets both conditions, `context` uses the matched search-line text instead, including ruby-stripped and cross-line forms. OCR matches keep their ±60-character window.
