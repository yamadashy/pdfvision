...
In regex mode two time budgets can turn a page's hits into an empty list: the ~1s per-page budget (that page's results are dropped) and the ~12s whole-request budget (that page and every page after it go unsearched, while earlier matches are kept). Both warn, both keep the result out of the cache, and both mean a `[]` on that page reports "not searched", not "not present" — which is why the warning, not the match count, is what settles a regex zero.
