...
export function shouldInsertSemanticSpace(prevText: string, curText: string, gap: number, fontSize: number): boolean {
  ...
  if (gap > fontSize * LATIN_WORD_SPACE_MIN_GAP_RATIO && LATIN_WORD_END_RE.test(prev) && NUMBER_START_RE.test(cur)) {
    return true;
  }
  ...
  if (
    gap > fontSize * LATIN_WORD_SPACE_MIN_GAP_RATIO &&
    ((LATIN_WORD_END_RE.test(prev) && GREEK_SYMBOL_START_RE.test(cur)) ||
      (GREEK_SYMBOL_END_RE.test(prev) && LATIN_WORD_START_RE.test(cur)))
  ) {
    return true;
  }
