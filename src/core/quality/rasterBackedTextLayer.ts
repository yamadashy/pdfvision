...
const NEAR_COMPLETE_PAGE_RASTER_COVERAGE_THRESHOLD = 0.98;
export const RASTER_BACKED_TEXT_LAYER_MAX_VECTOR_OPS = 12;
export const RASTER_BACKED_TEXT_LAYER_FULL_PAGE_MAX_VECTOR_OPS = 24;
...
export function isRasterBackedTextLayer(input: RasterBackedTextLayerInput): boolean {
  ...
  const rasterCoverage = maxRasterBackdropCoverage(input.imageBoxes, input.pageWidth, input.pageHeight);
  const vectorLimit =
    rasterCoverage >= NEAR_COMPLETE_PAGE_RASTER_COVERAGE_THRESHOLD
      ? RASTER_BACKED_TEXT_LAYER_FULL_PAGE_MAX_VECTOR_OPS
      : RASTER_BACKED_TEXT_LAYER_MAX_VECTOR_OPS;
  return (
    input.imageCount > 0 &&
    input.vectorCount <= vectorLimit &&
    hasEnoughText &&
    rasterCoverage >= FULL_PAGE_RASTER_COVERAGE_THRESHOLD
  );
}

function maxRasterBackdropCoverage(imageBoxes: readonly ImageBox[], pageWidth: number, pageHeight: number): number {
  ...
  for (const box of imageBoxes) {
    ...
    maxCoverage = Math.max(maxCoverage, overlap / pageArea);
  }
  return maxCoverage;
}
