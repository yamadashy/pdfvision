...
export interface ProcessOptions {
  ...
  /**
   * Format the output as a focused search report: the file, total
   * page/match counts, and flat emitted matches with page, query reference,
   * source, text, optional context, and bbox. The full pages/body payload
   * is omitted. Requires `search`; throws otherwise. Output size grows with
   * emitted matches and context.
   *
   * Purely a rendering concern (like {@link stripRepeated}): the
   * extraction is unchanged, only the formatter output differs. Applies
   * to all four formats. A run that finds nothing still succeeds and emits
   * a zero-match report — a zero count is a valid observation, not an error.
   */
  matchesOnly?: boolean;
  normalize?: boolean;
  geometry?: boolean;
